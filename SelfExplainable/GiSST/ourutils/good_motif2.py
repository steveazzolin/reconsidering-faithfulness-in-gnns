...
from ourutils.synthetic_data.BA3_loc import *
from ourutils.synthetic_data import synthetic_structsim
...
class GOODMotif2(InMemoryDataset):
    ...
    def _download(self):
        print(osp.join(self.raw_dir, self.name))
    ...
    def load(dataset_root: str, domain: str, shift: str = 'no_shift', generate: bool = False, debias: bool = False):
        ...
        id_val_dataset = GOODMotif2(root=dataset_root,
                                   domain=domain, shift=shift, subset='id_val', generate=generate,debias=debias) 
        id_test_dataset = GOODMotif2(root=dataset_root,
                                    domain=domain, shift=shift, subset='id_test', generate=generate,debias=debias) 
        ...
        #print("remember to fix this! load dataset")
        #return {'train': train_dataset,
                #'val': train_dataset, 'test': train_dataset}, meta_info

        return {'train': train_dataset,'val': id_val_dataset, 'test': id_test_dataset}, meta_info
